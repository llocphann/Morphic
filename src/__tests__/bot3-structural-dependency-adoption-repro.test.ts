...
import { RetainedSimpleStructuralOwnerSurfaceRegistry } from "../render/retained-production-simple-structural-owner";
...
interface ProductionInternals {
	...
	retainedStructuralOwnerSurfaces: unknown;
...
function createConfig(): ViewConfig {
	return {
		id: "structural-dependency-adoption",
		name: "Structural dependency adoption",
...
function installRejectingStructuralSurfaceRegistry(
	internals: ProductionInternals,
): RetainedSimpleStructuralOwnerSurfaceRegistry<TestOwner> {
	const backingRegistry = new RetainedSimpleStructuralOwnerSurfaceRegistry<TestOwner>(
		"obsidian-custom-view-render",
	);
	internals.retainedStructuralOwnerSurfaces = {
...
describe("Bot 3 structural retained dependency adoption transaction", () => {
	it("keeps Markdown last-known-good reverse edges when structural owner adoption fails", async () => {
		const restoreDom = installObsidianDomHelpers();
		try {
			const file = createFile("Notes/Structural.md");
			...
			installRejectingStructuralSurfaceRegistry(internals);
			...
			expect(() => transaction.commit()).toThrow(
				"Retained structural Markdown owner surface authority changed before adoption",
			);
			transaction.dispose?.();
	...
	it("keeps Canvas last-known-good reverse edges when structural owner adoption fails", async () => {
		const restoreDom = installObsidianDomHelpers();
		try {
			const file = createFile("Notes/Structural-Canvas.md");
			const config = createConfig();
			const { internals, invalidation } = createPlugin(createApp(file), file, config);
			...
			expect(() => transaction.commit()).toThrow(
				"Retained structural Canvas owner surface authority changed before adoption",
			);
			transaction.dispose?.();
