...
function createConfig(): ViewConfig {
	return {
		id: "dynamic-leaf-release-view",
		name: "Dynamic leaf release view",
		rules: { type: "group", operator: "AND", conditions: [] },
		template: "<article><span>{{ file.name }}</span><b>Stable</b></article>",
	};
}
...
describe("Bot 5 P5 production dynamic leaf owner authority", () => {
	it("retains Markdown DOM identity for a supported text expression leaf", async () => {
	...
	it("retains Canvas DOM identity for a supported text expression leaf", async () => {
		const restoreDom = installObsidianDomHelpers();
		try {
