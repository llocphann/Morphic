...
function createPlugin(app: App, file: TFile, config: ViewConfig): {
	...
	internals: ProductionPreparerInternals;
} {
...
describe("Bot 5 P5 production retained owner authority", () => {
	it("keeps Markdown static DOM identity across a same-structure semantic refresh", async () => {
	...
	it("keeps Canvas static DOM identity across a same-structure semantic refresh", async () => {
		const restoreDom = installObsidianDomHelpers();
		try {
